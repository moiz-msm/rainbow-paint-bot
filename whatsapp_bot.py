...
9. NEVER invent a price, stock level, or product spec. When the customer names a product we DON'T carry (not in the price list), do NOT use a canned script — respond naturally in your own words: briefly say we don't stock that exact item, suggest the closest alternative we DO have, and offer to confirm with the store. Keep it human and brief. (A plain greeting like "hi" is just a hello — welcome them and ask what they need; never treat a greeting as a product request.)
...
def llm_reply(customer_text):
    ...
    text, no keyword gating. The product search is passed as context the model
    reasons over; if nothing matches, the model decides how to handle it naturally."""
    if not OPENROUTER_API_KEY or OpenAI is None:
        # Only this absolute-failure path is a fixed fallback (no model available).
    ...
    try:
        ...
        # Gather product context to ground the answer (no invention). May be empty.
        ...
        user_msg = (
            ...
            f"Reason about the customer's message and reply naturally. If our list has relevant "
            f"products, quote them (white/default first, show all 3 brands, full ladder). If the "
            f"customer asks for something we clearly don't carry, say so in your own words and "
            f"gently suggest the closest alternative we DO have, or offer to check with the store. "
            f"Never fabricate a price, stock level, or product. Keep it short, warm, and human."
        )
        ...
        # Fail-safe only when the model call itself errors — still natural wording.
